...
$empresa = mysqli_query($link,$sql1);
	foreach ($empresa as $emp) {
		...
		$introducao = "<p style='font-size: 16px;'> O Presente estudo quando para fins previdenciários depende de duas definições, a nocividade e a permanência,
			demostrada através do Laudo Técnico das Condições Ambientais - <b><i>LTCAT</i></b>, para a empresa <b>".$emp['razaoSocial']."</b>, situada ".$emp['endereco'].", o mesmo tem como premissas, a realização de
...
$sql1 = "SELECT * FROM tbempresa_resp WHERE cdEmpresa = $id_empresa";
$resp = mysqli_query($link,$sql1);
	foreach($resp as $re){
	$rs_resp = $re['razaoSocial'];
	$unidade_resp = $re['unidade'];
	$endereco_resp = $re['endereco'];
	$cnpj_resp = $re['cnpj'];
	}
	
$sql1 = "SELECT * FROM tblocal WHERE cdEmpresa = $id_empresa";
$local = mysqli_query($link,$sql1);
	foreach($local as $loc){
	$rs_local = $loc['razaoSocial'];
	$unidade_local = $loc['unidade'];
	$endereco_local = $loc['endereco'];
	$cnpj_local = $loc['cnpj'];
	}
...
$ghe = mysqli_query($link,$sql1);
	$html .= '<tr style="text-align: center; ">
					<th colspan="4" style="color: white; background-color: rgb(0,32,92);"> Agente Ambientais</th>
			   </tr>
			   <tr style="text-align: center; background-color: rgb(180,198,231);">
			   		<th>Grupo</th>
			   		<th>Sub Grupo</th>
			   		<th>Cód. eSocial</th>
			   		<th>Agentes Ocupacionais</th>
			   	</tr>';
	foreach ($ghe as $ghe) { 
		$html .= '<tr style="text-align: center;">
					<td> '. $ghe['codGHE'] .' </td>';
	...
	$anteci_riscos .= '<table cellspacing=1 cellpadding=1 width="100%" style="font-size: 12px;"> 
		...
		if($risco -> num_rows == 0){
			...
			$anteci_riscos .= '<div style="height: 22cm; page-break-after: always;">
				...
				<table cellspacing=1 cellpadding=1 width="100%" style="font-size: 12px; text-align: center;"> 
					<tr>
			...
			$final = $final + 1;
			//resultado ruido
			$anteci_riscos .= '<div style="height: 22cm; page-break-after: always;  padding-top: 15px;">
				...
				<table cellspacing=1 cellpadding=1 width="100%" style="font-size: 10px; text-align: center; margin-bottom: 10px;"> 
					<tr>
				...
				<h2 style="font-size: 10px;  margin-bottom: 10px;"> IDENTIFICAÇÃO E RECONHECIMENTO GRUPO OCUPACIONAL: </h2>
				<table  cellpadding=1 width="100%" style="font-size: 12px;"> 
					<tr>
					    <td style="width: 30%; text-align: center;">Propagação / Absorção:</td>
						<td style="border: 1px solid black;"> '. $meioPropagacao .' </td>
				...
				<p style="font-size: 10px; text-indent: 0em; line-height: 1.3; margin: -20px 0px 0px 0px;"><b style="font-size: 11px;">RESUMO DE METODOLOGIA E INSTRUMENTAL UTILIZADO</b><br><b> '. $metodomedicao_agente .'</b> <br> <b> Equipamentos Utilizados na Medição </b> <br> '. $equipamento_medicao .' </p>';
						...
						$anteci_riscos .= '</tr></table><p style="font-size: 10px; text-indent: 0em; line-height: 1.3; margin: 0px; margin-bottom: 10px;">Nota:'.$obs.'</p>';
				...
				$final = $final + 1; //resultado vibracao
			...
			$anteci_riscos .= '<div style="height: 22cm; page-break-after: always;">
				...
				<table cellspacing=1 cellpadding=1 width="100%" style="font-size: 12px; margin-bottom: 10px; text-align: center;"> 
					<tr>
					    <td><b>CÓDIGO DO AGENTE - CÓDIGO E-SOCIAL</b></td>
					    <td></td>
						<td><b>Código do GHE</b></td>
				...
				<h2 style="font-size: 12px;  margin-bottom: 10px;"> IDENTIFICAÇÃO E RECONHECIMENTO GRUPO OCUPACIONAL: </h2>
				<table  cellpadding=1 width="100%" style="font-size: 12px; "> 
					<tr>
					    <td style="width: 30%; text-align: center;">Propagação / Absorção:</td>
						<td style="border: 1px solid black;"> '. $meioPropagacao .' </td>
...
$ghe = mysqli_query($link,$sql1);
	foreach ($ghe as $ghe) { 
		...
		$riscos_ambientais .= '<li> 10.'. $CODGHE .' - '. $nomeGHE .' </li>';
	}
...
$dompdf->load_html('
	...
	<div class="header">
		<table cellspacing=0 cellpadding=2>  
			<tr>
			    ...
			    <td style="width:370px; text-align: center; padding-left: 60px; font-size: 16px;">LTCAT - Laudo Técnico das Condições Ambientais</td>
	...
	<div class="footer">'.$footerperfil.'</span>
		...
		<div style="page-break-inside:always; height: 22cm;">
			<h3 style="text-align: center; font-size: 28px; margin: 150px 0px 0px 0px; padding-left: 15%; padding-right: 15%;">LAUDO TÉCNICO DE VALIAÇÕES DAS CONDIÇÕES AMBIENTAIS DOS LOCAIS DE TRABALHO<br>
			<h3 class="helvetica" style="text-align: center; font-size: 28px;">- LTCAT -</h3></h3>
			
			<h3 class="helvetica" style="text-align: center; font-size: 20px;  margin-top: 100px;"> '.$razao_social.' </h3>
			<h3 class="helvetica" style="text-align: center; font-size: 28px;  margin-top: 100px;"> - TESTE - </h3>
			<h3 class="helvetica" style="text-align: center; font-size: 16px;  margin-top: 100px;">Ipatinga - MG - 2019</h3>
			<h3 class="helvetica" style="text-align: center; font-size: 14px;">RT'.$id_empresa.'-'.$id_contrato.'</h3>
		</div>
		...
		<div style="page-break-after: always; height: 22cm; text-size: 10px; padding-left: 10px;">
			...
			<div style="margin-bottom: 4cm;">
					<div style="width: 100%;">
					<a><b>1. Introdução</b></a<div style="text-align: right; float: right; padding-top: -22px;">3</div></div>
					...
					<a><b>10. Metodologias de avaliação de agentes Ocupacional</b></a>
					<div style="text-align: right; float: right; padding-top: -22px;"></div>
					<ol style="padding-left: 15px; font-size: 12px; padding-top: 10px;">
				
								'. $riscos_ambientais .'
					</ol>
					<a><b>9. Resumo dos Agentes Avaliados</b></a>
					<div style="text-align: right; float: right; padding-top: -22px;"></div>
		...
		<div style="page-break-inside:always; height: 22cm;">
			...
			<p style="font-size: 16px;">Todos procedimentos para avaliação ambiental contida neste presente Laudo, estão de acordo com as
			metodologias das Normas de Higiene Ocupacional – <b>NHO – FUNDACENTRO</b>.</p>

			<p style="font-size: 16px;">Os agente ambientais serão apresentados com os códigos e nomenclaturas em conformidade o <i>decreto
			8373, de 2014 na qual foi estabelecido o</i> <b>eSocial.</b></p>
		</div>


		<div style="page-break-inside:always; height: 22cm; font-size: 12px;">
			<!-- 2 identificaçao -->
			<h3><a name="id_empresa">2. Identificação da Empresa </a></h3>		
			...
			<h3><a name="id_empresa">2.1 Identificação do Local da Prestação de Serviços </a></h3>		
			<ul style="list-style: none;">
			...
			<h3><a name="id_empresa">2.2 Identificação da Empresa Responsável pelo Estudo</a></h3>		
			<ul style="list-style: none;">
			...
			<p style="font-size: 13px;"> Conforme, estabelece o <b><i>art. 58 da lei nº 8.213, de 24 de julho de 1991</i></b>, que dispõe sobre os planos de benefícios da
			...
			física do trabalhador, para fins de concessão da aposentadoria especial.<br>
			Portanto, é necessário ressaltar que o LTCAT não possui a finalidade de caracterização e classificação da insalubridade
			e periculosidade, em atendimento as normas regulamentadoras do Ministério do Trabalho e Emprego – MTE.</p>


			<h3>4. Resumo de Atividades da Empresa </h3>
			
			<p> '. $resumo . '</p>
		</div>
		
		<div style="page-break-inside:always; height: 22cm; font-size: 20x;">	
		<!-- 5 Siglas e Abreviaturas -->
		<h2 style=" font-size: 18px;">5. Siglas e Abreviaturas </h2>
		<br>
			<p style="font-size: 14px; margin-top: -20px;">
			<b>MTE</b> - Ministério do Trabalho e Emprego
			<br<br><b>NR</b> – Normas Regulamentadoras
			<br<br><b>GFIP</b> - Guia de Recolhimento do FGTS
			<br<br><b>INSS</b> – Instituto Nacional do Seguro Social
			<br<br><b>FUNDACENTRO</b> – Fundação Jorge Duprat Figueiredo de Segurança e Medicina do Trabalho.
			<br<br><b>ACGIH</b> - American Conference of Governmental Industrial Hygienists.
			<br<br><b>ABHO</b> - Associação Brasileira de Higienistas Ocupacionais.
			<br<br><b>NHO</b> - Normas de Higiene Ocupacional - FUNDACENTRO
			<br<br><b>NIOSH</b> - National Institute for Occupational Safety and Health
			<br<br><b>OSHA</b> - Occupational Safety and Health Administration
			<br<br><b>ISO</b> - International Organization for Standardization
			<br<br><b>AIHA</b> - American Industrial Hygiene Association. Industrial Hygiene Statistics
			<br<br><b>TLV</b> - Threshold Limit Values (valor limite) AGIH
			<br<br><b>BEI</b> - Biological Exposure Indice (índice de exposição biológica) AGIH
			<br<br><b>TWA</b> – Threshold Limit Value – Time Weighted Average - Limite de tolerância ponderado no tempo- AGIH
			<br<br><b>TLV STEL</b> – Limite de exposição de curta duração (Short Term Exposure Limit). AGIH
			<br<br><b>EPI</b> – Equipamento de Proteção Individual.
			<br<br><b>EPC</b> – Equipamento de Proteção Coletiva.
			<br<br><b>CNEN</b> - Comissão Nacional de Energia Nuclear
			<br<br><b>CIPA</b> – Comissão Interna de Prevenção de Acidentes
			<br<br><b>CIPAMIN</b> – Comissão Interna de Prevenção de Acidentes na Mineração.
			<br<br><b>CNAE</b> – Classificação Nacional de Atividade Econômica.
			<br<br><b>CAT</b> – Comunicação de Acidente de Trabalho.
			<br<br><b>CLT</b> – Consolidação das Leis do Trabalho.
			<br<br><b>CA</b> – Certificado de Aprovação.
			<br<br><b>DDS</b> – Diálogo Diário de Segurança.
			<br<br><b>FISPQ</b> – Ficha de Segurança de Produtos Químicos.
			<br<br><b>CREA</b> – Conselho Regional de Engenharia e Agronomia.
			<br<br><b>CBO</b> – Classificação Brasileira de Ocupações.</p>
		
		</div>

		<div style="page-break-inside:always; height: 22cm; font-size: 15px">	
		<!-- 6 Bibliografia / Referência Técnicas -->
		<h2 style="font-size: 18px;">6. Bibliografia / Referência Técnicas </h2>
		<ul>
			<li style="margin-bottom: 10px;">BRASIL. Ministério do Trabalho e Emprego. Portaria nº 3.214, de 8 de junho de 1978. Aprova as normas
			...
			<li style="margin-bottom: 10px;"> ACGIH. <i>American Conference of Governmental Industrial Hygienists.</i> TLVs e BEIs: Limites de Exposição Ocupacional (TLVs) para Substâncias Químicas e Agentes Físicos e Índices Biológicos de Exposição (BEIs).</li>

			<li style="margin-bottom: 10px;"> FUNDACENTRO. Norma de Higiene Ocupacional NHO 1: Avaliação da Exposição Ocupacional ao Ruído.<br>
			São Paulo, 2001.</li>

			<li style="margin-bottom: 10px;"> FUNDACENTRO. Norma de Higiene Ocupacional NHO 6: Avaliação da Exposição Ocupacional ao Calor.<br>
			São Paulo, 2018.</li>

			<li style="margin-bottom: 10px;"> FUNDACENTRO. Norma de Higiene Ocupacional NHO 7: Calibração de Bombas de Amostragem Individual
			pelo Método da Bolha de Sabão. São Paulo, 2002.</li>

			<li style="margin-bottom: 10px;"> FUNDACENTRO. Norma de Higiene Ocupacional NHO 8: Coleta de Material Particulado Sólido Suspenso
			no Ar de Ambientes de Trabalho. São Paulo, 2007.</li>

			<li style="margin-bottom: 10px;"> FUNDACENTRO. Norma de Higiene Ocupacional NHO 9: Avaliação da exposição ocupacional a vibrações
			de corpo inteiro. São Paulo, 2007.</li>

			<li style="margin-bottom: 10px;"> FUNDACENTRO. Norma de Higiene Ocupacional NHO 10: Avaliação da exposição ocupacional a vibrações
			em mãos e braços.</li>

			<li style="margin-bottom: 10px;"> NIOSH. National Institute for Occupational Safety and Health. NIOSH Manual of Analytical <i>Methods</i> (NMAM).
			Publication 94-113. 4 ed. Atlanta: DHHS (NIOSH), 1994.</li>

			<li style="margin-bottom: 10px;"> OSHA. <i>Occupational Safety and Health Administration. Sampling and Analytical Methods. Methods
			Development Team: Industrial Hygiene Chemistry Division</i>, OSHA <i>Salt Lake Technical Center</i>.</li>

			<li style="margin-bottom: 10px;"> BRASIL. Presidência da República. Decreto nº 3.048, de 6 de maio de 1999. Aprova o Regulamento da
			Previdência Social, e dá outras providências.</li>

			<li style="margin-bottom: 10px;"> BRASIL. Ministério da Previdência Social. Instrução Normativa INSS/PRES Nº 77, de 21 de Janeiro de 2015.
			Dispõe sobre a administração de informações dos segurados.</li>

			<li style="margin-bottom: 10px;"> NIOSH. <i>National Institute for Occupational Safety and Health</i>. LEIDEL, Nelson A. et al. <i>Occupational
			Exposure Sampling Strategy Manual.</i> Ohio: NIOSH, 1977. 150p.</li>

			<li style="margin-bottom: 10px;"> AIHA. <i>American Industrial Hygiene Association. Industrial Hygiene Statistics</i>. Planilha eletrônica de
		...
		<h2 style="font-size: 18px;">7. Considerações Técnicas </h2>
			...
			<p>A publicação da <b><i>Lei nº 9.528, de 1997</b></i>, Introduz a obrigatoriedade da apresentação da GFIP, que passa a vigorar a partir de 1°/1/1999 no LTCAT com os seguintes códigos.</p>
			<div style="margin-left: 30px">
			
				<p><i>(em branco) — Sem exposição a agente nocivo. Trabalhador nunca esteve exposto.
				<br><b>01</b> — Não exposição a agente nocivo. Trabalhador já esteve exposto.
				<br><b>02</b> — Exposição a agente nocivo (aposentadoria especial aos 15 anos de trabalho);
				<br><b>03</b> — Exposição a agente nocivo (aposentadoria especial aos 20 anos de trabalho);
				<br><b>04</b> — Exposição a agente nocivo (aposentadoria especial aos 25 anos de trabalho).
				<br>Para os trabalhadores com mais de um vínculo empregatício
				<br><b>05</b> — Não exposto a agente nocivo;
				<br><b>06</b> — Exposição a agente nocivo (aposentadoria especial aos 15 anos de trabalho);
				<br><b>07</b> — Exposição a agente nocivo (aposentadoria especial aos 20 anos de trabalho);
				<br><b>08</b> — Exposição a agente nocivo (aposentadoria especial aos 25 anos de trabalho).</i></p>
			</i>
			</div>
		...
		<div style="page-break-inside:always; height: 22cm;">	
		<!-- 9 Metodologias de avaliação de agentes Ocupacional -->
		<h2 style="font-size: 18px;">9. Metodologias de avaliação de agentes Ocupacional </h2>
			<h4><b>Metodologia para avaliação da exposição ocupacional ao Ruído (ANEXOS 1 e 2 da NR-15 e NHO-01)</b></h4>
			<p>As avaliações da exposição ocupacional ao ruído estão fundamentadas nas especificações estabelecidas no Anexo 1 da Norma Regulamentadora - NR-15 da Portaria 3.214/78 do Ministério do Trabalho e Emprego – MTE a qual estabelece Limite de Tolerância para jornada de até oito horas diárias para o ruído contínuo ou intermitente correspondente a 85 dB(A) e valor teto igual a 115 dB(A).
			<br>Foram adotados os critérios e procedimentos de avaliação da exposição ocupacional ao ruído preconizados pela Norma de Higiene Ocupacional - NHO-01 da FUNDACENTRO, mantendo o incremento de duplicação da dose, fator “q=5”), em conformidade com o Anexo 1 da NR-15.
			<br>Segundo a NHO-01, a dose referente a uma jornada diária de trabalho corresponde ao parâmetro utilizado para a caracterização da exposição ocupacional ao ruído, expresso em porcentagem de energia sonora, tendo como referência o valor máximo da energia sonora diária admitida, definida com base nos seguintes parâmetros:
			<br><b>Critério de Referência (CR)</b>: o nível de ruído para o qual a exposição, por um período de oito horas, corresponde a uma dose de 100%. 
			<br><b>Incremento de Duplicação da Dose (Fator “q”)</b>: incremento em decibéis que, quando adicionado a um determinado nível, implica a duplicação da dose de exposição ou a redução para a metade do tempo máximo de exposição permitido.
			<br><b>Nível Limiar de Integração (NLI)</b>: corresponde ao nível de ruído a partir do qual os valores devem ser computados na integração para fins de determinação de nível médio ou da dose de exposição. <br> O critério de referência que embasa os Limite de Tolerância definidos no Anexo 1 da NR-15, para ruído contínuo ou intermitente corresponde a uma dose de 100% para exposição de 8 horas ao nível de 85 dB(A). O critério de avaliação considera, além do critério de referência, o incremento de duplicação da dose (Fator “q = 5”) e o nível limiar de integração igual a 80 dB(A). A avaliação da exposição ocupacional ao ruído contínuo ou intermitente foi executada por meio da determinação da dose diária de ruído e do nível de exposição, parâmetros estes representativos da exposição diária do empregado.<br>
			...
			<a style="font-size: 20px;">D = Te/480 x 100 x 2^</a><a style="font-size 8px;">(ne-85/5)</a>[%]<span>  </span><a style="font-size: 20px;">NE = 16,61 x log (480/Te x D/100) + 85 [db]</a>

			<p>Onde:</p>
			<p><b>NE</b> = nível de eposição<br>
			<b>D</b> = dose diária de ruído em porcentagem<br>
			<b>TE</b> = tempo de duração, em minutos, da jornada diária de trabalho</p>
			<h4><b> INSTRUMENTOS UTILIZADOS </b></h4>
		...
		<div style="page-break-inside:always; height: 22cm;">
			<h4><b> LIMITES DE TOLERÂNCIA ADOTADOS </b></h4>
			<p>Foram adotados os Limites de Tolerância estabelecidos no Anexo 1 da Norma Regulamentadora -
			NR-15 da Portaria 3.214/78 do Ministério do Trabalho e Emprego - MTE conforme a tabela abaixo:</p>
				<p><center><b> NR15 - ANEXO I</b></center></p>
				<p><center><b> LIMITES DE TOLERÂNCIA PARA RUÍDO CONTÍNUO OU INTERMITENTE</b></center></p>
				<br>
				<div style="margin-left: 25%;">
					<table border="1" cellspacing=0 cellpadding=1 >
							<tr>
								<th><b>NÍVEL DE RUÍDO DB(A)</b></th>
								<th><b>MÁXIMA EXPOSIÇÃO DIÁRIA PERMISSÍVEL</b></th>
							</tr>
							<tr>
								<td>85</td>
								<td>8 horas</td>
							...
							<tr>
								<td>96</td>
								<td>1 hora e 45 minutos</td>
							</tr>
							<tr>
								<td>98</td>
								<td>1 hora e 15 minutos</td>
							</tr>
							<tr>
								<td>100</td>
								<td>1 hora</td>
			...
			<h5><b>CRITÉRIO DE INTERPRETAÇÃO DOS RESULTADOS</b></h5>
			<p style="margin-top: 20px;">Para fins de comparação com o Limite de Tolerância, determinamos o Nível de Exposição Normalizado (NEN) o qual corresponde ao nível de exposição, convertido para uma jornada padrão de 8 horas diárias.
O (NEN) é determinado pela seguinte expressão:</p>
			<h3>NEN = NR + 10 Log (TE / 480) [dB]</h3>
			<p>Onde:<br>
			<b>NE</b> = nível médio representativo da exposição ocupacional diária<br>
			<b>TE</b> = tempo de duração, em minutos, da jornada diária de trabalho<br>
			De acordo com este critério, o Limite de Tolerância ao ruído corresponde a NEN igual a 85 dB(A), e o valor teto para ruído contínuo ou intermitente é de 115 dB(A).<br>
			Para interpretação dos resultados obtidos adotamos o critério e as considerações técnicas preconizadas na NHO-01, item 6.6.1.3 conforme segue:</p>
			<br>
			<div style="margin-left: 10%; margin-right: 15%;">
				<table border="1"  cellspacing=0 cellpadding=2 style="width: 100%;">
					<tr>
						<th>Fator "q"</th>
						<th>NEN db(a)</th>
						<th>Consideração Técnica Quanto à Exposição Ocupacional</th>
					</tr>
					<tr>
						<td rowspan="4">5</td>
						<td>< 80</td>
						<td>Exposição abaixo do nível de ação</td>
					</tr>
					<tr>
						<td>>=80 <85</td>
						<td>Exposição no nível de ação e abaixo do Limite de Tolerância</td>
					</tr>
					<tr>
						<td>= 85</td>
						<td>Exposição igual ao Limite de Tolerância</td>
					</tr>
					<tr>
						<td>> 85</td>
						<td>Exposição acima do Limite de Tolerância da NR15</td>
					</tr>
				</table>
			</div>
			<h4><b>CRITÉRIO DE AVALIAÇÃO DA ATENUAÇÃO DOS PROTETORES AUDITIVOS:</b></h4>
			<p>Para determinação da atenuação efetiva dos protetores auditivos em uso nos locais de trabalho estudados e indicação de protetores auditivos, quando necessário, aplicou-se a seguinte expressão:</p>
			<h4><b>NEN (dBA) – NRRsf (dBA) = NEN Atenuado(dBA).</b></h4>
			<p style="font-size: 14px;">Esta metodologia, baseia-se na norma ANSI S12.6 - 1997 (B).</p>
			<br>
		</div>
		<div style="page-break-inside:always; height: 22cm;">
			<h4><b>Metodologia para avaliação da exposição ocupacional a Vapores Ácidos (ANEXO 11 da NR-15,NR-09 e ACGIH)</b></h4>
			<p>As coletas foram executadas através de amostradores ativos (cassete de poliestireno de 37 mm, de três seções,com filtro de fibra de vidro tratado com carbonato de sódio e suporte precedido por filtro de fibra de vidro) fabricados pela SKC. O amostrador foi conectado suporte de baixa vazão e bomba gravimétrica com volume de ar total correspondente ao que é determina o métodos NIOSH específicos para coleta em seguida foi instalado na altura da zona respiratória dos empregados avaliados. A vazão considerada corresponde à média aritmética simples entre a vazão inicial e final obtidas, tudo Conforme determina os métodos NIOSH específicos.
			<h4><b>INSTRUMENTOS UTILIZADOS</b></h4>
			<p>Para determinação do volume de ar, expresso em metro cúbico (m³), foram utilizadas bombas gravimétricas (de aspiração contínua) - marca: Gillian - modelo: BDX II. Estas bombas gravimétricas foram previamente calibradas com a utilização de um calibrador de vazão eletrônico,- marca: Gilian - modelo: Gilibrator 2.
			</p>
			<h4><b>CRITÉRIO DE INTERPRETAÇÃO DOS RESULTADOS</b></h4>
			<p>Os valores das Concentrações Médias obtidas dos ácidos em estudo foram relacionados com os Limites de Tolerância estabelecidos no Anexo 11 da NR-15 ou ACGIH, através da relação direta entre eles, chamada de Concentração Relativa (CR) que é dada pela seguinte fórmula:
			</p>
			<h3><b>CR = C / (LT ou TLV)</b></h3>
			<p><b>Onde:</b>
				<br><b>CR</b> = Concentração Relativa
				<br><b>C</b> = Concentração do agente ambiental obtida
				<br><b>LT</b> ou <b>TLV</b> = Limite de Tolerância ou Valor Limite de Exposição
				<br>A interpretação dos valores da Concentração Relativa obedece ao seguinte critério:
			</p>
			<br>
			<center><table border="1" cellspacing=0 cellpadding=2 style="width: 100%;">
				<tr>
					<th>Valor da Concentração Relativa</th>
					<th>Interpretação relacionada à concentração do agente ocupacional</th>
				</tr>
				<tr>
					<td>CR< 0,5</td>
					<td>Concentração Abaixo do Limite de Tolerância ou TLV</td>
				</tr>
				<tr>
					<td>CR>=0,5 < 1</td>
					<td>Concentração acima do nível de ação e abaixo do Limite ou TLV</td>
				</tr>
				<tr>
					<td>CR=1</td>
					<td>Concentração igual do Limite de Tolerância ou TLV</td>
				</tr>
				<tr>
					<td>CR>1</td>
					<td>Concentração acima do Limite de Tolerância ou TLV</td>
				</tr>
			</table></center>
		</div>
		<div style="page-break-inside:always; height: 22cm;">	
			<h4 style="padding-bottom: -10px;"><b>Metodologia para avaliação da exposição ocupacional a Poeiras Minerais (ANEXO 12 da NR-15, NR-09, NHO 08, NIOSH e ACGIH)</b></h4>
			<p>As coletas foram executadas sobre filtros de membranas de PVC 37 mm de diâmetro e 5 micrometros de porosidade fabricadas pela SKC e montadas em porta-filtro apropriados do mesmo fabricante. O conjunto de filtro e porta-filtro, que constitui a chamada cabeça de amostragem, foi fixado em suporte especial e instalado na altura da zona respiratória dos empregados avaliados.
			<br>
				Foi adotado o método analítico gravimétrico para determinação da massa de particulados do ar, expresso em “mg” (miligramas) pelo qual utilizamos uma balança analítica de precisão da marca Mettler com sensibilidade de 0,01 mg.
			<br>
				A vazão considerada corresponde à média aritmética simples entre a vazão inicial e final obtidas, tudo conforme determina os métodos estandardizados. As coletas de Poeira Total em campo foram preparadas e executadas em conformidade com o método NIOSH 0500 adaptado para NIOSH 7500 - NHO 08 Exceto Tabela I do Anexo B. As coletas de Poeira Respirável foram preparadas e executadas em conformidade com o método NIOSH 7500 e NHO-08 Exceto Tabela I do Anexo B, utilizando seletor de partículas (cliclone).</p>
			<h4 style="padding-bottom: -10px; padding-top: -10px;"><b>INSTRUMENTOS UTILIZADOS</b></h4>
			<p>Para determinação do volume de ar, expresso em metro cúbico (m³), foram utilizadas bombas gravimétricas (de aspiração contínua), procedência americana - marca: Gillian - modelo: BDX II.
			<br>
				Estas bombas gravimétricas foram previamente calibradas com a utilização de um calibrador de vazão eletrônico, procedência americana - marca: Gilian - modelo: Gilibrator 2.
			<br>
				Para as avaliações de Poeira Respirável, o amostrador (cassete + ciclone) foi calibrado dentro de um vasilhame chamado de “jarro” ou “câmara de calibração”.
			<h4 style="padding-bottom: -10px; padding-top: -10px;"><b>ESPECIFICAÇÃO DOS LIMITES DE TOLERÂNCIA ADOTADOS</b></h4>
			<p>Foram adotados os Limites de Tolerância estabelecidos no Anexo 12 da Norma Regulamentadora - NR-15 da Portaria 3.214/78 do Ministério do Trabalho e Emprego - MTE e para os particulados com Limites de Tolerância omissos pela NR-15 foram adotados os TLV’s preconizados pela ACGIH com a finalidade de manter maior rigor científico.
			<br>
				O Limite de Tolerância para Poeiras Minerais contendo sílica livre cristalizada foi estabelecido conforme o critério estabelecido no Anexo 12 da Norma Regulamentadora - NR-15 da Portaria 3.214/78 do Ministério do Trabalho e Emprego - MTE, a partir dos percentuais de SiO2 (dióxido de silício) obtidos nas amostras, foram determinados através das seguintes fórmulas:
			</p>
			<h4 style="padding-bottom: -10px; padding-top: -10px;"><b>Poeira Total:</b></h4>
			<p><center>LT = _____24_______<br>
			% de quartzo + 3</center></p>
			<h4 style="padding-bottom: -10px; padding-top: -10px;"><b>Poeira Respirável:</b></h4>
			<p><center>LT = ______8_______<br>
			% de quartzo + 2</center></p>
			<p><i>Obs.: Para as massas coletadas onde não foram observados percentuais de sílica livre cristalizada SiO2 acima do limite de quantificação do método analítico empregado e de acordo com as normas de análise laboratórial, os materiais passaram a ser classificado, para fins deste trabalho, como </i><i><b>Particulados (insolúveis ou de baixa solubilidade) não especificados de outra maneira (PNOS) - respiráveis<b> da ACGIH.</i>
			</p>
			<h4><b>CRITÉRIO DE INTERPRETAÇÃO DOS RESULTADOS</b></h4>
			<p>
				Os valores das Concentrações Médias obtidas do particulado em estudo foram relacionados com os Limites de Tolerância estabelecidos no Anexo 12 da NR-15 ou ACGIH, através da relação direta entre eles, chamada de Concentração Relativa (CR) que é dada pela seguinte fórmula:
			</p>
			<h3><center><b>CR = C / (LT ou TLV)</b></center></h3>
			<p style="margin-top: -10px;">
				<b>Onde:</b><br>
				<b>CR</b> = Concentração Relativa<br>
				<b>C</b> = Concentração do particulado obtida<br>
				<b>LT</b> ou <b>TLV</b> = Limite de Tolerância ou Valor Limite de Exposição<br>
				<br>
				<center>A interpretação dos valores da Concentração Relativa obedece ao seguinte critério:</center>
			</p>
			<center><table border="1" cellspacing=0 cellpadding=2 style="width: 100%;">
				<tr>
					<th>Valor da Concentração Relativa</th>
		...
		<div style="page-break-inside:always; height: 22cm;">	
		<!-- 10 Resumo dos Agentes Avaliados -->
		<h3>10. Resumo dos Agentes Avaliados </h3>
			<h4> <b> AGENTES AMBIENTAIS AVALIADOS: </b> </h4>
		...
		<div style="page-break-inside:always; height: 22cm;">
			<h2 style="text-align: center; font-size: 18px;">11. Resultados e Conclusões </h2>
		'. $anteci_riscos .'
		<div style="height: 22cm; page-break-after: always;">
